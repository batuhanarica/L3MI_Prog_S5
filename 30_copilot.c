...
// Ajoute un watcher pour un littéral donné dans une clause
void add_watch(Formula* f, Clause* c, size_t lit_idx) {
    lit l = c->lits[lit_idx];
    var v = abs(l);

    WatchNode* w = malloc(sizeof(WatchNode));
    w->clause = c;
    w->lit_idx = lit_idx;
    ...
    for (size_t idx = from; idx < f->nassigned; idx++) {
        ...
        while (*ptr) {
            ...
            if (val != 1) { // le littéral n'est pas vrai → on cherche à le remplacer
                ...
                for (size_t i = 0; i < c->size; i++) {
                    if (i != c->wid1 && i != c->wid2) {
                        ...
                        if (eval != 0) { // non faux
                            ...
                            // Move Watcher
                            *ptr = node->next; // Remove from current list

                            lit new_lit = c->lits[i];
                            var new_var = abs(new_lit);
                            node->lit_idx = i;

                            if (new_lit > 0) {
                                node->next = f->vars[new_var].pos;
                                f->vars[new_var].pos = node;
                            } else {
                                node->next = f->vars[new_var].nega;
                                f->vars[new_var].nega = node;
                            }
                ...
                if (replaced) {
                    continue; // *ptr is already next node
                } else {
                    int8_t clause_val = clause_eval(f, c);
...
void fonctionresolve(Formula* f) {
    ...
    while (!formula_eval(f)) {
        ...
        while (!propagation_ok) {
            ...
            // Essayer l'assignation inverse (0)
            ...
            if (!propagation_ok) {
                // Si propagation échoue encore, on doit annuler cette assignation aussi
                backtrack(f, last);
                // Et on continue à backtracker (la boucle while continue)
            }
        }
    }
...
Formula* read_cnf(const char* fname)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        cidx++;
        // Ajouter les watchers pour les deux littéraux surveillés
        add_watch(res, c, c->wid1);
        add_watch(res, c, c->wid2);
    }
